...
The GUI visual shell is a polished dashboard: `.app-shell` contains a persistent `.brand-rail` (logo lockup, local device identity, then a bottom-anchored Settings button directly above the footer) plus a `.dashboard` content area for directories, devices, and conflicts. The rail header is the stacked logo with a "Synche" `.brand-wordmark` below it; the footer is just the `© … • v{{ version }}` line (no GitHub/source link). The palette is the five-color brand set defined as raw vars (`--text`, `--background`, `--primary`, `--secondary`, `--accent`) in light `:root` and the dark `@media (prefers-color-scheme: dark)` block (there is no `data-theme` toggle). **Every semantic token (`--brand-*`, `--network-*`, `--accent-amber*`, surfaces, text, borders) is derived from those five via `color-mix()` with `var(--text)` as the contrast pole, so the same formula auto-adjusts across themes — do not reintroduce raw hex into the semantic tokens.** Anchors: primary green is light `#beff99` / dark `#256600`. Green (`--primary`) is for brand, primary actions, and healthy sync states; the blue `--accent` is the network/device accent (`--network-*`); the teal `--secondary` drives activity/warnings (`--accent-amber*`). Red is the one intentionally-retained non-palette hue for destructive/conflict states (`--danger-*`); shadows/backdrops keep neutral translucent-black `rgba()`. `--on-accent` is `var(--text)` so button labels stay readable on the primary fill in both themes. The two logo `<img>`s swap via `.brand-logo-on-light`/`.brand-logo-on-dark` classes, each hidden by a `@media (prefers-color-scheme: …)` query (not a base `display:none` + override, which is source-order-fragile, and not the `hidden` attr). For contrast, light theme shows the dark-green `icon-dark.svg` and dark theme shows the bright `icon.svg`. Local device IDs in the rail render as compact `.id-chip` values with the full UUID preserved in the DOM/title. Preserve light/dark token pairs, high-contrast text, visible focus states, aligned dashboard panel geometry, and responsive rules when changing the UI; do not collapse the app back to a monochromatic theme or a card-style top header.
