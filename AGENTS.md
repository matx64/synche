...
SQLite reads that map rows into `EntryInfo` must select explicit domain columns (`name, kind, hash, version, deleted`) rather than `SELECT *`. Persistence-only columns such as `tombstoned_at` are intentionally outside the domain type, and explicit selects avoid stale or migration-time schema shapes from leaking into sqlx row mapping.
